...
async function uploadAsset(
  ...
  const filePath = path.join(appDataPath, 'assets', asset.file_name);
...
async function publishToMakerWorld(
  ...
  const uploadedAssets: { original: { name: string; url: string }[]; images: { name: string; url: string }[]; models: { name: string; url: string; size: number }[] } = {
    original: [],
    images: [],
    models: [],
  };
  ...
  const draftData = {
    ...
    modelFiles: uploadedAssets.models.map(model => ({
      ...
      unikey: Math.random().toString(36).slice(2),
      thumbnailName: '',
      thumbnailSize: 0,
      thumbnailUrl: '',
      modelUpdateTime: '',
...
async function recordMakerWorldStatus(designId: string, platformDesignId: string | number, status: string) {
  await fetch('/api/makerworld/model/record', {
    method: 'POST',
